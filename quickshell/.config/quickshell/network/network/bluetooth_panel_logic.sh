...
# Ensure glyphs/icons are emitted as UTF-8 for QML rendering.
if [[ -z "${LANG:-}" ]]; then
    export LANG=C.UTF-8
fi
if [[ -z "${LC_ALL:-}" ]]; then
...
get_icon() {
    ...
    elif [[ "$type" == *"audio"* || "$type" == *"speaker"* || "$type" == *"card"* || "$name" == *"speaker"* ]]; then echo ""
...
get_status() {
    ...
    if [ "$power" == "on" ]; then
        ...
        for c_line in "${connected_info_lines[@]}"; do
            ...
            echo "CACHE_NAME=\"$CACHE_NAME\"" > "$CACHE_FILE"
            echo "CACHE_ICON=\"$CACHE_ICON\"" >> "$CACHE_FILE"
            echo "CACHE_PROFILE=\"$CACHE_PROFILE\"" >> "$CACHE_FILE"
